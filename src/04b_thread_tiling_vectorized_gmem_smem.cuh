// 04b_thread_tiling_vectorized_gmem_smem.cuh
// Thread tiling SGEMM kernel with vectorized global and shared memory access, transposed As
...
__global__ void sgemm_thread_tiling_vectorized_gmem_smem(
    ...
    const float *A, const float *B, float beta, float *C)
{
    ...
    for (uint tileIdx = 0; tileIdx < K; tileIdx += BK) {
        ...
        // Compute outer products (vectorized smem reads)
        for (uint k = 0; k < BK; ++k) {
...
template<int BM, int BN, int BK, int TM, int TN>
struct SGEMMThreadTilingVectorizedGmemSmem {
    static void Run(int M, int N, int K, float alpha,
        ...
        sgemm_thread_tiling_vectorized_gmem_smem<BM, BN, BK, TM, TN><<<grid, block>>>(M, N, K, alpha, A, B, beta, C);
